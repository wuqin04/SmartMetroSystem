...
public class TXTFileManager implements FileManager {
	...
	public void saveData(Object data, String fileName) throws FileProcessingException {
		try (PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {
	        ...
	        if (data instanceof List<?>) {                    // Bug 1: check type instead of assuming List
	        	List<?> list = (List<?>) data;
	        	for (Object obj : list) {
	        		writer.println(obj.toString());
	        	}
	        } else if (data instanceof Map<?, ?>) {           // Bug 1: also support HashMap
	        	Map<?, ?> map = (Map<?, ?>) data;
	        	for (Object value : map.values()) {
	        		writer.println(value.toString());
	        	}
	        } else {
	        	throw new FileProcessingException("Unsupported data type for " + fileName);
	        }

			System.out.println("[SUCCESS] Data saved successfully");
		}catch(IOException e) {
			throw new FileProcessingException("Failed to save data to " + fileName);
		}
	...
	public Object loadData(String fileName) throws FileProcessingException {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
				...
				throw new FileProcessingException("Fail to load data from " + fileName);
			}
